    ...
    <div>
        <div id="nationalEmissionRate">
            ...
            <i18n path="emissionRateBody.text" tag="p">
                <template #MWhInfo>
                    <a @click="$parent.$parent.showMegaWattInfo = true" href="javascript:void(0)">{{ $t("emissionRateBody.MWhInfo") }}</a>
                </template>
                <template #eGRIDSubregions>
                    <a href="https://epa.gov/sites/production/files/styles/large/public/2020-03/2018_egrid_subregions.png" target="_blank">{{ $t("emissionRateBody.eGRIDSubregions") }}</a>
                </template>
                <template #co2>
                    <a href="/ghgemissions/overview-greenhouse-gases#carbon-dioxide" target="_blank">{{ $t("emissionRateBody.co2") }} (CO<sub>2</sub>)</a>
                </template>
                <template #so2>
                    <a href="/so2-pollution" target="_blank">{{ $t("emissionRateBody.so2")}} (SO<sub>2</sub>)</a>
                </template>
                <template #nox>
                    <a href="/no2-pollution" target="_blank">{{ $t("emissionRateBody.nox") }} (NO<sub>X</sub>)</a>
                </template>
            </i18n>
            ...
            <div id="sortingDiv">
                <p id="nationalEmissionRateSortingStatus"><strong>This chart is sorted alphabetically A → Z by region.</strong></p>
                <a href="javascript:void(0)" @click="handleSortLink" id="sortAlphabeticallyEmissionRate">A → Z Sort regions alphabetically</a>
                <a href="javascript:void(0)" @click="handleSortLink" id="sortAscendingEmissionRate">↑ Sort values ascending</a>
                <a href="javascript:void(0)" @click="handleSortLink" id="sortDescendingEmissionRate">↓ Sort values descending</a>
            </div>
        </div>
        <h3>Maps</h3>
        <div>
...
export default {
    ...
    methods: {
        ...
        displayHorizontal: function(){
            ...
            for (var i = 0; i < self.parameters.length; i += 1) {
                ...
                container.append("rect")
                    ...
                    .attr("y", function(d) { return y(d.properties.emissionFactor[self.parameters[i]].value); })
                    .attr("width", x.rangeBand())
                    .attr("height", function(d) { return height - y(d.properties.emissionFactor[self.parameters[i]].value); })
                    .on("click", function(d){
                ...
                var nationalContainer = svg.selectAll(".nationalContainer")
                    .data(self.nationalFeature)
                    .enter().append("g")
                    .attr("class","emissionRateTooltip")
                    .attr("title",function(d){
                ...
                nationalContainer.append("rect")
                    ...
                    .attr("fill", function(d) { return color(d.properties.name );})
                    .attr("x", function(d){ return width + x.rangeBand(); })
                    .attr("y", function(d){ return y(d.properties.emissionFactor[self.parameters[i]].value); })
                    .attr("width", x.rangeBand())
                    .attr("height", function(d) { return height - y(d.properties.emissionFactor[self.parameters[i]].value); })
                    .on("click", function(d){
                ...
                nationalBar.append("text")
                    ...
                    .attr("x", function(d) { return width + x.rangeBand() + x.rangeBand()/2;})
                    .attr("y", function(d) { return y(d.properties.emissionFactor[self.parameters[i]].value) + -10;})
                    .text(function(d){
                ...
                if(Object.keys(self.selectedRegion).length) {
                    d3.selectAll(self.domElement + " .tick text")
                    ...
                    svg.selectAll("text.allEmissionRate")
                        ...
                        .attr("y", function(d) {
                            return y(d.properties.emissionFactor[self.parameters[i]].value) - 10;
                        })
                        .text(function(d){
                            return d.properties.emissionFactor[self.parameters[i]].display;
                        });
            ...
            for (var i = 0; i < self.parameters.length; i += 1) {
                ...
                svg.append("text")
                    ...
                    .append("tspan")
                        ...
                        .append("tspan").text(" Rate (lbs/MWh)");
                ...
                container.append("rect")
                    ...
                    .attr("width", function(d) { return x(d.properties.emissionFactor[self.parameters[i]].value); })
                    .on("click", function(d){
                        if(self.sortState == "ascending"){
                            $("#sortDescendingEmissionRate")[0].click()
                        } else if(self.sortState == "descending"){
                ...
                nationalBar.append("text")
                    .attr("text-anchor","middle")
                    .attr("class","selectedRegionValue")
                    .attr("y", function(d) { return height + y.rangeBand() - 3; })
                    .attr("x", function(d) { return x(d.properties.emissionFactor[self.parameters[i]].value) + 20;})
                    .text(function(d){
                        return d.properties.emissionFactor[self.parameters[i]].display;
                    });
                ...
                if(Object.keys(self.selectedRegion).length) {
                    ...
                    svg.selectAll("text.allEmissionRate")
                        ...
                        .attr("x", function(d) {
                            return x(d.properties.emissionFactor[self.parameters[i]].value) + 25;
                        })
                        .text(function(d){
                            return d.properties.emissionFactor[self.parameters[i]].display;
                        });
        ...
        showSelection: function() {
            ...
            $("#all" + this.selectedPollutantAll).siblings("svg").hide();
        },
        hide: function() {
        ...
        sort: function(sortState, selectedPollutantAll) {
            ...
            if(sortState == "alphabetically") {
                ...
                data.sort(function(x,y){
                    return d3.ascending(x.properties.emissionFactor[selectedPollutantAll].value, y.properties.emissionFactor[selectedPollutantAll].value);
                });
            } else if (sortState == "descending") {
                data.sort(function(x,y){
                    return d3.descending(x.properties.emissionFactor[selectedPollutantAll].value, y.properties.emissionFactor[selectedPollutantAll].value);
                });
            } else if (sortState == "subregionFuels") {
                data.sort(function(x,y){
                    return d3.descending(x.properties.fuelMix[self.selectedFuel], y.properties.fuelMix[self.selectedFuel]);
                });
            }
        },
        update: function(){
        ...
        updateStatus: function(pollutant, state){
            if(state == "ascending" || state == "descending"){
                return "<strong>This chart is sorted by " + state + " " + pollutant + " emission rates.</strong>"
        ...
        handleSortLink: function(e){
            ...
            if(sortVal == "sortAlphabeticallyEmissionRate") {
                ...
                var status = self.updateStatus(self.pollutantText, userSelection.sortState)
                $(el).hide();
                $(el).siblings().show();
                $("#nationalEmissionRateSortingStatus").html(status);
                $("#nationalFuelMixSortingStatus").html(status);
