...
export default {
    ...
    data() {
        return {
            ...
            // Fuels
            fuels : ["gas","coal","nuclear","hydro","wind","biomass","solar","oil","geothermal","otherFossilFuel","otherUnknownFuel"],
            renewableAndNon: ["renewable","non-renewable"],
            renewableNonNuclearAndHydro: ["hydro","renewable (excluding hydro)","non-renewable (excluding nuclear)","nuclear"],
            // Colors
            allFuelsColorRange: ["#fdae61","#313695","#f46d43","#74add1","#abd9e9","#4575b4","#ffffbf","#d73027","#fee090","#e0f3f8","#a50026"],
            renewableAndNonColors: ["#abdda4","#d7191c"],
            renewableNonNuclearAndHydroColors: ["#2b83ba","#abdda4","#d7191c","#fdae61"],
            subregions: {},
            sortedFuel: "",
    ...
    methods: {
        ...
        display: function(selectedFuelCategory){
            ...
            if(this.sortState == "alphabetically") {
                ...
                this.subregionData.sort(function(x,y){
                    return d3.ascending(x.properties.emissionFactor[self.$children[0].selectedPollutantAll].value, y.properties.emissionFactor[self.$children[0].selectedPollutantAll].value);
                });
            } else if (this.sortState == "descending") {
                this.subregionData.sort(function(x,y){
                    return d3.descending(x.properties.emissionFactor[self.$children[0].selectedPollutantAll].value, y.properties.emissionFactor[self.$children[0].selectedPollutantAll].value);
                });
            } else if (this.sortState == "subregionFuels") {
                this.subregionData.sort(function(x,y){
                    return d3.descending(x.properties.fuelMix[selectedFuel], y.properties.fuelMix[selectedFuel]);
                });
            }
        ...
        displayHorizontal: function(colors, fuelBreakdown, fuelKey){
            ...
            var fuels = subregion.selectAll("g")
            ...
            var legendBottom = svg.selectAll(".legendBottom")
                .data(color.domain())
                .enter().append("g")
            ...
            if(self.sortedFuel) self.sortHorizontalBarChart(self.sortedFuel, duration, fuelKey, x, y, subregion, xAxis);
        ...
        sortHorizontalBarChart(fuel, duration, fuelKey, x, y, subregion, xAxis){
            ...
            var selectedFuelArr = [];
            var sortedSubregionData = allSubregions.data.sort(
        ...
        displayVertical: function(colors, fuelBreakdown, fuelKey){
                ...
                .attr("transform", function(d, i){
                    ...
                    if(self.selectedFuelCategory == "renewableNonNuclearAndHydro"){
                        ...
                        // Third row
                            ...
                            return "translate(" + (midLen + offsetL * 2 + 20) + "," + (-100) + ")";
                        } else if (i === 7) {
                            return "translate(" + (midLen + offsetL * 2 + 20) + "," + (-80) + ")";
                        } else if (i === 8) {
                            return "translate(" + (midLen + offsetL * 2 + 20) + "," + (-60) + ")";
                        } else if (i === 9) {
                ...
                lgLabels.append("tspan")
                    .text(function(d){
                        ...
                        if (d == "otherFossilFuel") {
                            return sarr[0].charAt(0).toUpperCase(1) + sarr[0].substr(1) + " " + sarr[1];
                        } else {
            ...
            var duration = {'sub':300,'x':0};
            if(self.sortedFuel) self.sortVerticalBarChart(self.sortedFuel, duration, fuelKey, x, y, subregion, yAxis);
            addLogoBottom(svg, width - 60, height + 50);
        },
        sortVerticalBarChart: function(fuel, duration, fuelKey, x, y, subregion, yAxis){
            var self = this;
            self.sortedFuel = fuel;
            userSelection.sortedFuel = self.sortedFuel
            $("#resetNationalFuelMix").css("visibility","visible");
            ...
            var svg = d3.select(self.domElement)
            var subregion = svg.selectAll(".subregion")
                .sort(function(a, b) { return y0(a.subregion) - y0(b.subregion); });
        ...
        handleFuelClick: function(fuel, fuelKey, x, y, subregion, xAxis, yAxis, orientation){
            var self = this
            d3.selectAll(".textselected").style("font-weight", function(d){
                if(d == fuel.name) {
                    return "bold";
            ...
            if(orientation == "horizontal"){
                this.sortHorizontalBarChart(fuel, duration, fuelKey, x, y, subregion, xAxis);
            } else {
                this.sortVerticalBarChart(fuel, duration, fuelKey, x, y, subregion, yAxis);
            }
            self.sortState = "subregionFuels";

            $("#sortingDiv > a").show();
            $("#nationalFuelMixSortingStatus").html("<strong>This chart is sorted by decreasing % of " + self.selectedFuel + ".");
            $("#nationalEmissionRateSortingStatus").html("<strong>This chart is sorted by decreasing % of " + self.selectedFuel + ".");
        ...
        handleSortReset: function(){
            ...
            $("#nationalEmissionRateSortingStatus").html("<strong>This chart is sorted alphabetically A → Z by region.</strong>");
            $("#nationalFuelMixSortingStatus").html("<strong>This chart is sorted alphabetically A → Z by region.</strong>");
        },
        addHovertext: function(fuel, fuelName, f){
            ...
            return formatFuelLabel(fuelName) + " accounts for " + f(fuel.val) + "% of the fuel mix " + checkNational(fuel.subregion)
        },
        updateChild: function(sortState, fuel){
            var emRateChart = this.$children[0]
            if(fuel){ emRateChart.selectedFuel = fuel.name}
