...
export default {
    ...
    methods: {
        display: function(data) {
            ...
            var h = 500;
            var allFuelsColorRange = ["#fdae61","#313695","#f46d43","#74add1","#abd9e9","#4575b4","#ffffbf","#d73027","#fee090","#e0f3f8","#a50026"]
            var fuels = ["gas","coal","nuclear","hydro","wind","biomass","solar","oil","geothermal","otherFossilFuel","otherUnknownFuel"]
            ...
            x.domain(data.map(function(d){ return d.properties.name; }));
            y.domain([0,100]);
            ...
            subregion.selectAll("rect")
                ...
                .attr("title", function(fuel){
                    return formatFuelLabel(fuel.name) + " accounts for " + f(fuel.val) + "% of the fuel mix " + checkNational(fuel.subregion)
                })
                .attr("height", function(d) { return y(d.y0) - y(d.y1); })
                .style("fill", function(d) { return color(d.name); });
        
            var legendBottomSub = svg.selectAll(".legendBottomSub")
                .data(color.domain())
                .enter().append("g")
            ...
            legendBottomSub.append('text')
                .attr("x", 20)
                .attr("y", 10)
                .text(function (d, i) {
                    if(d != "otherFossilFuel" && d != "otherUnknownFuel"  && d != "geothermal") {
                        return formatFuelLabel(d) + " (" + data[1].properties.fuelMix[d] + "%)";
                    }
                })
            ...
            d3.selectAll(domElement + " .legendBottomSub text").style("font-size","0.7em");
