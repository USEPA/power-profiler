<template>
    <div id="emissionsCalculator">
        ...
        <div>
            ...
            <div>
                <p>Or use the <a v-on:click.prevent="displayNationalAverage" href="javascript:void(0)" id="nationalAverageLink" class="emissionsLink">national average electricity use</a>.</p>
            </div>
        </div>
...
import { addLogoBottom, formatFuelLabel, checkNational } from "../helpers/ChartHelpers.js"
...
export default {
    ...
    mounted: function(){
        ...
        $(window).width() > 950 ? this.emissionsResultsWidth = $(window).width() / 4 : this.emissionsResultsWidth = 250
    ...
    methods: {
        displayMonthlyAverage: function(){
            ...
            function calculateEmissions(emissionFactorValue, monthlyAverage) {
                var res = ((monthlyAverage * 12) * emissionFactorValue * 0.001) * (1 + self.gridLoss.value);
                return res;
            }

            this.userEmissions = {
                "co2": calculateEmissions(this.emissionFactors.co2EmissionRate.value, userMonthlyAverage),
                "so2": calculateEmissions(this.emissionFactors.so2EmissionRate.value, userMonthlyAverage),
                "nox": calculateEmissions(this.emissionFactors.noxEmissionRate.value, userMonthlyAverage)
            };

            this.subregionEmissions = {
                "co2": self.calculateSubregionEmissions(this.emissionFactors.co2EmissionRate.value, this.gridLoss.value),
                "so2": self.calculateSubregionEmissions(this.emissionFactors.so2EmissionRate.value, this.gridLoss.value),
                "nox": self.calculateSubregionEmissions(this.emissionFactors.noxEmissionRate.value, this.gridLoss.value)
            };

            this.nationalEmissions = {
                "co2": self.calculateNationalEmissions(this.nationalFeature[0].properties.emissionFactor.co2EmissionRate.value),
                "so2": self.calculateNationalEmissions(this.nationalFeature[0].properties.emissionFactor.so2EmissionRate.value),
                "nox": self.calculateNationalEmissions(this.nationalFeature[0].properties.emissionFactor.noxEmissionRate.value)
            };
            ...
            $("#egrid-subregion-acronym").html(this.selectedSubregion.properties.name);
            $("#egrid-subregion-name").html(this.selectedSubregion.properties.fullName);
            ...
            var percentMax = 100;
            var userPercent =  (((userMonthlyAverage * 12) / (this.nationalAverage * 12)) * 100);
            ...
            this.gaugeChart(this.emissionsResultsWidth,this.emissionsResultsHeight / 2, gaugeMax, userMonthlyAverage * 12, "Your Annual", "Electricity Use",false);
            this.gaugeChart(this.emissionsResultsWidth,this.emissionsResultsHeight / 2, percentMax, userPercent, "Your Electricity Use", "Compared to the National Average",true);
            d3.selectAll("#resultGraphs svg").remove();
            d3.selectAll("#resultGraphs-rpt svg").remove();
            this.displayUserAndNationalEmissions(this.userEmissions, this.subregionEmissions, this.nationalEmissions, "#resultGraphs", this.emissionsResultsWidth, this.emissionsResultsHeight);
        ...
        displayMonthlyActual: function(){
            ...
            var vals = $("#calculateMonthlyActualForm input").map(function() {
                return $(this).val();
            }).get();
            ...
            function calculateEmissions(emissionFactorValue, total) {
                var res = ((total * emissionFactorValue) / 1000) * (1 + self.gridLoss.value);
                return res;
            }

            this.nationalEmissions = {
                "co2": this.calculateNationalEmissions(this.nationalFeature[0].properties.emissionFactor.co2EmissionRate.value),
                "so2": this.calculateNationalEmissions(this.nationalFeature[0].properties.emissionFactor.so2EmissionRate.value),
                "nox": this.calculateNationalEmissions(this.nationalFeature[0].properties.emissionFactor.noxEmissionRate.value)
            };

            this.subregionEmissions = {
                "co2": this.calculateSubregionEmissions(this.emissionFactors.co2EmissionRate.value, this.gridLoss.value),
                "so2": this.calculateSubregionEmissions(this.emissionFactors.so2EmissionRate.value, this.gridLoss.value),
                "nox": this.calculateSubregionEmissions(this.emissionFactors.noxEmissionRate.value, this.gridLoss.value)
            };

            this.userEmissions = {
                "co2": calculateEmissions(this.emissionFactors.co2EmissionRate.value, total),
                "so2": calculateEmissions(this.emissionFactors.so2EmissionRate.value, total),
                "nox": calculateEmissions(this.emissionFactors.noxEmissionRate.value, total)
            };
            ...
            $("#pounds-of-so2").html(userSulfur);
            $("#pounds-of-nox").html(userNitrogen);
            $("#no-of-tree-seedlings").html(this.calculateCarbonOffset(parseFloat(this.userEmissions.co2)).trees);
            $("#acres-of-forests").html(this.calculateCarbonOffset(parseFloat(this.userEmissions.co2)).acres);
        ...
        displayNationalAverage: function(e){
            ...
            function calculateEmissions(emissionFactorValue, monthlyAverage) {
                var res = ((monthlyAverage * 12) * emissionFactorValue / 1000) * (1 + self.gridLoss.value);
                return res;
            }

            this.nationalEmissions = {
                "co2": calculateEmissions(this.nationalFeature[0].properties.emissionFactor.co2EmissionRate.value, this.nationalAverage),
                "so2": calculateEmissions(this.nationalFeature[0].properties.emissionFactor.so2EmissionRate.value, this.nationalAverage),
                "nox": calculateEmissions(this.nationalFeature[0].properties.emissionFactor.noxEmissionRate.value, this.nationalAverage)
            };

            this.subregionEmissions = {};

            this.userEmissions = {
                "co2": calculateEmissions(this.emissionFactors.co2EmissionRate.value, this.nationalAverage),
                "so2": calculateEmissions(this.emissionFactors.so2EmissionRate.value, this.nationalAverage),
                "nox": calculateEmissions(this.emissionFactors.noxEmissionRate.value, this.nationalAverage)
            };
            ...
            if(this.residentialMode == false) {
                ...
                this.gaugeChart(this.emissionsResultsWidth,this.emissionsResultsHeight / 2, gaugeMax, this.nationalAverage * 12, "Your Annual", "Electricity Use",false);
            ...
            $("#nat-egrid-subregion-acronym").html(this.selectedSubregion.properties.name);
            $("#nat-egrid-subregion-name").html(this.selectedSubregion.properties.fullName);
            $("#nat-percent-line-loss").html(this.gridLoss.display);
            $("#nat-estimated-annual-electricity-use").html((this.nationalAverage * 12).toLocaleString());
            $("#nat-pounds-of-co2").html(userCarbon);
            $("#nat-pounds-of-so2").html(userSulfur);
            $("#nat-pounds-of-nox").html(userNitrogen);
            $("#nat-no-of-tree-seedlings").html(this.calculateCarbonOffset(parseFloat(this.userEmissions.co2)).trees);
            $("#nat-acres-of-forests").html(this.calculateCarbonOffset(parseFloat(this.userEmissions.co2)).acres);
            ...
            $("#commercialCustomersForm").on("submit", function(e){
                ...
                $("#nat-em-rpt-intro-2").html("estimated from the national average electricity consumption of 1.22 kWh/sq. ft./month for commercial customers and");
            });
            ...
            $("#residentialCustomersButton").click(function(){
                ...
                $("#nationalAverageLink").trigger("click");
                $("#nat-em-rpt-intro-2").html("estimated from the average home consumption of 1,011 kWh/month and");
            });
        ...
        gaugeChart: function(width, height, maxVal, num, text1, text2, percent){
            ...
            .styleTween("fill", function() { return d3.interpolate(cur_color, cur_color); })
            .call(arcTween, numPi);

            d3.selectAll("#chart-gauge svg text")
            .style("font-family","'Source Sans Pro', 'Helvetica Neue', 'Helvetica', 'Roboto', 'Arial', sans-serif");

            d3.selectAll("#chart-gauge svg .y.axis path, #resultGraphs svg .axis line")
                .style("fill", "none")
                .style("stroke", "#000")
        ...
        calculateCarbonOffset: function(carbon) {
            ...
            var trees = (carbon / (23.2 * (44 / 12))).toLocaleString(undefined, {maximumFractionDigits: 0});
            var acres = (carbon / (0.85 * 2204.6)).toLocaleString(undefined, {maximumFractionDigits: 0});
            return {"trees": trees, "acres": acres};
        },
        displayUserAndNationalEmissions: function(user, subregion, national, domElement, w, h){
            ...
            for (var i = 0; i < pollutants.length; i += 1) {
                ...
                var arr = [
                    {
                        ...
                        "display": Number(parseFloat(national[pollutants[i]]).toFixed(0)).toLocaleString(),
                        "name": "National Average"
                    },
                    {
                        "value": parseFloat(subregion[pollutants[i]]),
                        "display": Number(parseFloat(subregion[pollutants[i]]).toFixed(0)).toLocaleString(),
                        "name": this.selectedSubregion.properties.name + " Average"
                    },
                    {
                        "value": parseFloat(user[pollutants[i]]),
                        "display": Number(parseFloat(user[pollutants[i]]).toFixed(0)).toLocaleString(),
                        "name": "Your Emissions"
                    }
                ];
                ...
                var cArr = [this.emRatesColors.national,this.emRatesColors.subregion,this.emRatesColors[pollutants[i] + "EmissionRate"]];

                if(this.resultsFunction == "nationalAverage") {
                    arr.splice(1,1);
                    cArr = [this.emRatesColors.national, this.emRatesColors[pollutants[i] + "EmissionRate"]];
                }
                ...
                x.domain(arr.map(function(d){ return d.name; }));
                y.domain([0,d3.max(arr, function(d){ return d.value; })]);
                ...
                var labels = svg.selectAll("text.bar")
                    .data(arr)
                    .enter().append("text");
            ...
            d3.selectAll(domElement +  " svg .y.axis path, " + domElement + " svg .axis line")
                .style("fill", "none")
                .style("stroke", "#000")
                .style("shape-rendering","crispEdges");
