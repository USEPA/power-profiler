...
df["Percent Non-renewable Excluding Nuclear (coal, oil, gas, other fossil, other unknown) net generation"] = df["Percent Non-renewable Excluding Nuclear (coal, oil, gas, other fossil, other unknown) net generation"].round(1)
...
# Add national data
row = ["U.S.", "National", n["USTRPR"].values[0], n["USTNPR"].values[0], (n["USTNPR"] - n["USNCPR"]).round(1).values[0], n["USTHPR"].values[0], n["USNCPR"].values[0], n["USHYPR"].values[0]]
df.loc[26] = row
# Write to .csv file
